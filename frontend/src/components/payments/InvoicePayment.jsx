...
const InvoicePayment = ({ onBack, onHome, invoiceData }) => {
  const { user } = useAuth();
  const [formData, setFormData] = useState({
    paymentRef: 'Pay/25/0002',
    paymentType: 'Receive',
    partnerType: 'Customer',
    partner: invoiceData?.customerName || 'Dinesh Pathak',
    amount: invoiceData?.amount || 23610,
    date: new Date().toISOString().split('T')[0],
    journal: 'Default Bank can be selectable to Cash',
    note: 'Alpha numeric ( text )'
  });
  ...
  const handlePrint = () => {
    console.log('Printing Invoice Payment');
  };

  const handleSend = () => {
    console.log('Sending Invoice Payment');
  };

  const handleCancel = () => {
    setStatus('Cancelled');
    console.log('Invoice Payment cancelled');
  };
